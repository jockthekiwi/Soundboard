...
gulp.task("build-misc", () => {
    ...
    let soundsFilter = filter("**/*.{mp3,mp4,wav,flac}", { restore: true });
...
gulp.task("build-index", () => {
    return gulp
        .src(config.index)
        .pipe(inject(
            gulp.src(`${config.buildDir}/**/*.{js,css}`, { read: false }), { ignorePath: config.buildDir, addRootSlash: false, removeTags: true, quiet: true }
        ))
